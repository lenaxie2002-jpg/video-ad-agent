...
function buildSegmentPrompt({ prompt, product, selectedMaterials, totalDuration, segmentIndex, segmentCount, segmentDuration }) {
  return [
    ...
    segmentIndex === 0 ? '本段是开场：3 秒内展示该商品本体和真实使用场景，并且商品尺寸必须与详情页一致。' : '',
...
${productConstraintText(resolvedProduct, selectedMaterials)}` : '';
  const visualLock = `

【视觉锁定】本次视频必须基于随请求传入的 Shopify 商品参考图片生成。参考图 URL 数量：${refs.length}。请保持同一个商品的外观、颜色、材质、轮廓、结构、尺寸比例、安装方式和设计细节一致。不要把商品替换成随机木纹、抽象纹理、其它家具或其它灯具。`;
  const finalPrompt = `${visualLock}${productContext}

【用户生成要求】${prompt}`.slice(0, 5200);

  const normalizedDuration = normalizeOpenRouterDuration(duration);
  const job = createJob('openrouter-video-generation', { sku, prompt: finalPrompt, duration: normalizedDuration, ratio, resolution, product: resolvedProduct, referenceImages: refs, selectedMaterials });
  ...
  process.nextTick(async () => {
    try {
      ...
      await generateOpenRouterVideoAndSave({ localJobId: job.id, prompt: finalPrompt, duration: normalizedDuration, ratio, resolution, model: videoModel, product: resolvedProduct, referenceImages: refs, selectedMaterials });
    } catch (error) {

...
async function generateOpenRouterVideoAndSave({ localJobId, prompt, duration, ratio, resolution, model, product }) {
  ...
  updateJob(localJobId, { status: 'saving_to_drive', progress: 98, result: { localVideoPath: merged.localVideoPath, segments: segmentResults.map((s) => ({ openRouterJobId: s.openRouterJobId })) } });
...
async function pollOpenRouterVideoAndSave(localJobId, submittedJob) {
  ...
  updateJob(localJobId, { status: 'saving_to_drive', progress: 98, result: { openRouterJobId: single.openRouterJobId, localVideoPath } });
  ...
  updateJob(localJobId, { status: 'completed', progress: 100, result: { openRouterJobId: single.openRouterJobId, videoUrl: `/api/jobs/${localJobId}/video`, localVideoPath, driveFile } });
}

...
function sanitizeFilename(name = 'document.md') {
  return String(name).replace(/[^a-zA-Z0-9._\-\u4e00-\u9fff]/g, '_').slice(0, 120) || 'document.md';
}

function normalizeMarkdownText(text, maxChars = 80000) {
  const value = String(text || '').replace(/^\uFEFF/, '').trim();
  return value.length > maxChars ? value.slice(0, maxChars) + '\n\n[TRUNCATED]' : value;
}

function getUploadedMdText(ids = []) {
  if (!Array.isArray(ids)) return '';
  return ids.map((id) => uploadedMdDocs.get(String(id))).filter(Boolean).map((doc) => `# ${doc.filename}\n${doc.content}`).join('\n\n---\n\n');
}

function combineUserRequirements({ basePrompt = '', requirementText = '', mdText = '', mdDocumentIds = [] } = {}) {
  const blocks = [];
  if (basePrompt) blocks.push(`【用户当前视频生成要求】\n${basePrompt}`);
  if (requirementText) blocks.push(`【前端需求输入框内容】\n${requirementText}`);
  if (mdText) blocks.push(`【前端直接传入MD内容】\n${mdText}`);
  const uploaded = getUploadedMdText(mdDocumentIds);
  if (uploaded) blocks.push(`【用户上传的MD需求文档】\n${uploaded}`);
  return normalizeMarkdownText(blocks.join('\n\n'), 100000);
}

app.post('/api/upload-md', upload.single('file'), asyncHandler(async (req, res) => {
  if (!req.file) return res.status(400).json({ ok: false, error: 'file is required' });
  const filename = sanitizeFilename(req.file.originalname || 'document.md');
  if (!/\.(md|markdown|txt)$/i.test(filename)) {
    return res.status(400).json({ ok: false, error: '只支持 .md / .markdown / .txt 文件' });
  }
  const content = normalizeMarkdownText(req.file.buffer.toString('utf8'), 120000);
  if (!content) return res.status(400).json({ ok: false, error: 'MD 文件为空' });
  const id = uuidv4();
  const doc = {
    id,
    filename,
  ...
  uploadedMdDocs.set(id, doc);
  res.json({ ok: true, document: { id: doc.id, filename: doc.filename, size: doc.size, preview: doc.preview, uploadedAt: doc.uploadedAt } });
}));

app.get('/api/upload-md/:id', asyncHandler(async (req, res) => {
  const doc = uploadedMdDocs.get(String(req.params.id));
  if (!doc) return res.status(404).json({ ok: false, error: 'MD document not found' });
...
app.post('/api/agent/create-image', asyncHandler(async (req, res) => {
  const { sku, prompt, ratio = '1:1', resolution = '1024x1024', product, imageModel, requirementText = '', mdText = '', mdDocumentIds = [] } = req.body;
  ...
  const imagePrompt = combineUserRequirements({ basePrompt: prompt, requirementText, mdText, mdDocumentIds });
  const result = await openRouterGenerateImage({ prompt: imagePrompt, model: imageModel, ratio, resolution, product: resolvedProduct });
...
app.post('/api/agent/create-video', asyncHandler(async (req, res) => {
  const { sku, prompt, duration = 30, ratio = '16:9', resolution = '1080p', product, videoModel, imageUrl, referenceImages, selectedMaterials = [], requirementText = '', mdText = '', mdDocumentIds = [] } = req.body;
  if (!prompt) return res.status(400).json({ ok: false, error: 'prompt is required' });
  ...
  const combinedPrompt = combineUserRequirements({ basePrompt: prompt, requirementText, mdText, mdDocumentIds });
  ...
  const job = createJob('openrouter-video-generation', { sku, prompt: finalPrompt, duration: normalizedDuration, ratio: normalizedRatio, resolution: normalizedResolution, product: resolvedProduct, referenceImages: refs, selectedMaterials, knowledgeVersion: 'md-pack-v2', mdDocumentIds });

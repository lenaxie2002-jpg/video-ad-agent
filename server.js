...
async function generateOpenRouterVideoAndSave({ localJobId, prompt, duration, ratio, resolution, model, product }) {
  ...
  if (!shouldSegment) {
    ...
    const openRouterCreate = await createOpenRouterVideoJob({ prompt, duration: finalDuration, ratio, resolution, model: selectedModel });
    ...
    for (let index = 0; index < segmentCount; index += 1) {
      ...
      const openRouterCreate = await createOpenRouterVideoJob({ prompt: segmentPrompt, duration: thisDuration, ratio, resolution, model: selectedModel });
...
app.post('/api/agent/create-video', asyncHandler(async (req, res) => {
  ...
  process.nextTick(async () => {
    try {
      updateJob(job.id, { status: 'starting_30s_ad_pipeline', progress: 5, result: { product: resolvedProduct, videoModel: videoModel || optional('OPENROUTER_VIDEO_MODEL', 'bytedance/seedance-2.0') } });

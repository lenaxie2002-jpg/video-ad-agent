(function () {
  ...
  const STORAGE_KEY = 'video-ad-agent-real-session-v3';
  let isRestoringResults = false;
  const stateBag = {
    product: null,
    lastJob: null,
    lastVideoUrl: null,
  ...
  function serializableState() {
    return {
      product: stateBag.product,
      lastJob: stateBag.lastJob,
      lastVideoUrl: stateBag.lastVideoUrl,
      lastDriveFile: stateBag.lastDriveFile,
      lastImage: stateBag.lastImage,
      videoJobs: stateBag.videoJobs,
      videoResults: stateBag.videoResults,
      selectedMaterials: stateBag.selectedMaterials,
      mdDocuments: stateBag.mdDocuments,
      requirementText: stateBag.requirementText,
      savedAt: new Date().toISOString()
    };
  }

  function persistState() {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(serializableState())); } catch (_) {}
  }
  ...
  function hydrateState() {
    try {
      ...
      ['product','lastJob','lastVideoUrl','lastDriveFile','lastImage','videoJobs','videoResults','selectedMaterials','mdDocuments','requirementText'].forEach((key) => {
        if (saved[key] !== undefined && saved[key] !== null) stateBag[key] = saved[key];
      });
    } catch (_) {}
  }
  ...
  function renderMdUploadListReal() {
    const list = document.getElementById('md-upload-list');
    if (!list) return;
    if (!stateBag.mdDocuments.length) {
      list.innerHTML = '<div class="upload-item"><i data-lucide="file-text" style="width:16px;height:16px;color:var(--muted);"></i><div><strong>尚未上传真实 MD 文件</strong><span>上传后会进入后端，并在生成图片/视频时参与 prompt。</span></div></div>';
      return;
    }
    list.innerHTML = stateBag.mdDocuments.map((doc) => `
      <div class="upload-item real-md-doc" data-md-id="${doc.id}">
        <i data-lucide="file-text" style="width:16px;height:16px;color:var(--muted);"></i>
        <div>
          <strong>${doc.filename}</strong>
          <span>已上传到后端 · ${fileSizeLabel(doc.size)} · 会参与生成</span>
        </div>
        <div class="actions">
          <span class="tag tag-success">已接入后端</span>
          <button class="btn btn-ghost btn-sm" type="button" data-remove-md="${doc.id}">移除</button>
        </div>
      </div>`).join('');
  }

  function readRequirementText() {
    const text = document.getElementById('md-inline-text');
    stateBag.requirementText = text ? text.value.trim() : stateBag.requirementText || '';
    const docs = stateBag.mdDocuments.map((doc) => `${doc.filename}: ${doc.preview || ''}`).join('\n\n');
    return [stateBag.requirementText, docs ? `上传MD摘要：\n${docs}` : ''].filter(Boolean).join('\n\n');
  }

  async function uploadMdFile(file) {
    if (!file) return;
    if (!/\.(md|markdown|txt)$/i.test(file.name || '')) {
      toast('只支持上传 .md / .markdown / .txt 文件', 'warn');
      return;
    }
    const form = new FormData();
    form.append('file', file);
    toast(`正在上传 MD：${file.name}`);
    const res = await fetch('/api/upload-md', { method: 'POST', body: form });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data.ok === false) throw new Error(data.error || `上传失败：${res.status}`);
    stateBag.mdDocuments.push(data.document);
    persistState();
    renderMdUploadListReal();
    toast(`MD 已上传并接入生成：${data.document.filename}`, 'ok');
  }

  function setupMdUploadBridge() {
    const input = document.getElementById('md-upload-input');
    const zone = input && input.closest('.upload-zone');
    renderMdUploadListReal();
    if (input && !input.dataset.realMdBound) {
      input.dataset.realMdBound = '1';
      input.addEventListener('change', async (event) => {
        event.preventDefault();
        event.stopImmediatePropagation();
        const files = Array.from(event.target.files || []);
        for (const file of files) {
          try { await uploadMdFile(file); }
          catch (err) { toast(`MD 上传失败：${err.message}`, 'error'); }
        }
        input.value = '';
      }, true);
    }
    if (zone && !zone.dataset.realMdDropBound) {
      zone.dataset.realMdDropBound = '1';
      zone.addEventListener('dragover', (event) => { event.preventDefault(); zone.style.borderColor = 'var(--accent,#0071e3)'; });
    ...
    const text = document.getElementById('md-inline-text');
    if (text && !text.dataset.realMdTextBound) {
      text.dataset.realMdTextBound = '1';
      text.addEventListener('input', () => { stateBag.requirementText = text.value.trim(); persistState(); });
    }
  }
  ...
  function updateCandidateWithJob(job, index = 0, silent = false) {
    syncCandidateCardsWithCurrentChoices();
    ...
    if (job.status === 'completed' && job.result) {
      ...
      if (!silent) toast(job.result.driveFile ? `${variantLabel(index)} 已生成并保存到 Drive` : `${variantLabel(index)} 已生成；Drive 保存失败但可预览/下载`, job.result.driveFile ? 'ok' : 'warn');
    }
    if (job.status === 'failed' && !silent) toast(`${variantLabel(index)} 生成失败：${job.error}`, 'error');
    persistState();
    updateOverallBanner();
  }
  ...
  async function startRealImageGeneration() {
    try {
      ...
      const payload = {
        ...
        requirementText: readRequirementText(),
        mdDocumentIds: stateBag.mdDocuments.map((doc) => doc.id)
      };
  ...
  function restoreGeneratedResultsFromState() {
    if (isRestoringResults) return;
    isRestoringResults = true;
    try {
    const hasAny = (stateBag.videoJobs && stateBag.videoJobs.length) || (stateBag.videoResults && stateBag.videoResults.length) || stateBag.lastVideoUrl;
    if (!hasAny) return;
    if (stateBag.product) updateProductUI(stateBag.product);
    if (stateBag.product) renderShopifyMaterials(stateBag.product);
    const jobs = stateBag.videoJobs || [];
    jobs.forEach((job, index) => {
      if (job) updateCandidateWithJob(job, index, true);
    });
    if (!jobs.length && stateBag.lastVideoUrl) {
      updateCandidateWithJob({ status: 'completed', progress: 100, result: { videoUrl: stateBag.lastVideoUrl, driveFile: stateBag.lastDriveFile } }, 0, true);
    }
    updateOverallBanner();
    } finally {
      window.setTimeout(() => { isRestoringResults = false; }, 120);
    }
  }

  function scheduleRestoreGeneratedResults(delay = 120) {
    window.setTimeout(() => restoreGeneratedResultsFromState(), delay);
  }
  ...
  function install() {
    ...
    setupMdUploadBridge();
    if (stateBag.requirementText && document.getElementById('md-inline-text')) document.getElementById('md-inline-text').value = stateBag.requirementText;
    if (stateBag.product) { updateProductUI(stateBag.product); renderShopifyMaterials(stateBag.product); }
    scheduleRestoreGeneratedResults(250);
    ...
    document.addEventListener('click', (event) => {
      ...
      if (btn.dataset && btn.dataset.removeMd) {
        event.preventDefault();
        event.stopPropagation();
        stateBag.mdDocuments = stateBag.mdDocuments.filter((doc) => doc.id !== btn.dataset.removeMd);
        persistState();
        renderMdUploadListReal();
        toast('已移除该 MD 文档，本次生成将不再使用它。', 'warn');
        return;
      }
    ...
    document.addEventListener('click', (event) => {
      const nav = event.target.closest('[data-screen="04"], #btn-next, #btn-prev');
      if (nav) scheduleRestoreGeneratedResults(250);
    }, true);

    const resultGrid = document.getElementById('video-result-grid');
    if (resultGrid) {
      const observer = new MutationObserver(() => {
        if (!isRestoringResults && currentScreenId() === '04') scheduleRestoreGeneratedResults(180);
      });
      observer.observe(resultGrid, { childList: true, subtree: true });
    }
  ...
  window.RealVideoAgent = { startRealVideoGeneration, startRealImageGeneration, searchShopify, saveMetadataToDrive, renderShopifyMaterials, updateSelectedMaterialUI, state: stateBag, setupMdUploadBridge, persistState, restoreGeneratedResultsFromState };

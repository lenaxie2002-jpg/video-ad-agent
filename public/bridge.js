(function () {
  ...
  function setCandidateLoading(job) {
    ...
    if (banner) {
      banner.innerHTML = `<div class="banner processing"><div><div class="banner-title">真实视频生成中</div><div class="banner-body">已提交到后端任务 ${job.id}。本次按你在前一步选择的 ${currentDuration()} 秒、${currentRatio()}、${currentResolution()} 生成；如模型不支持单次生成，后端会自动分段合成。</div></div><div class="progress-bar"><span id="real-job-progress" style="width:5%;"></span></div></div>`;
    }
  ...
  function updateCandidateWithJob(job) {
    ...
    if (banner) {
      ...
      const body = job.status === 'failed'
        ? (job.error || '未知错误')
        : job.status === 'completed' && job.result && job.result.driveError
          ? `视频已生成，可预览/下载；但保存到 Google Drive 失败：${job.result.driveError}`
          : `状态：${job.status} · 进度：${job.progress || 0}%`;
      const actionHtml = job.status === 'completed' && job.result
        ? `<div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end;">${job.result.videoUrl ? `<a class="btn btn-primary btn-sm" target="_blank" href="${job.result.videoUrl}">打开视频</a>` : ''}${job.result.driveFile ? `<a class="btn btn-outline btn-sm" target="_blank" href="${job.result.driveFile.webViewLink}">打开 Drive 文件</a>` : ''}</div>`
        : `<div class="progress-bar"><span id="real-job-progress" style="width:${job.progress || 0}%;"></span></div>`;
      banner.innerHTML = `<div class="banner ${job.status === 'failed' ? '' : 'processing'}"><div><div class="banner-title">${title}</div><div class="banner-body">${body}</div></div>${actionHtml}</div>`;
    }
    if (job.status === 'completed' && job.result) {
      ...
      if (first) {
        ...
        if (tag) {
          tag.className = job.result.driveFile ? 'tag tag-success' : 'tag tag-warning';
          tag.textContent = job.result.driveFile ? 'Drive 已保存' : '视频已生成';
        }
        const info = $('.summary-list', first);
        if (info) {
          info.innerHTML = `<div><strong>真实生成结果:</strong> OpenRouter 视频已完成，可以在上方预览。</div>${job.result.driveFile ? `<div><strong>Drive:</strong> <a href="${job.result.driveFile.webViewLink}" target="_blank">${job.result.driveFile.name}</a></div>` : ''}${job.result.driveError ? `<div><strong>Drive 保存失败:</strong> ${job.result.driveError}</div>` : ''}`;
        }
      }
      toast(job.result.driveFile ? '真实广告视频已生成并保存到 Google Drive' : '真实广告视频已生成；Drive 保存失败，但视频可预览/下载', job.result.driveFile ? 'ok' : 'warn');
    }
  ...
  async function startRealVideoGeneration() {
    try {
      ...
      syncCandidateCardsWithCurrentChoices();
      toast(`正在提交真实视频生成任务：${currentDuration()} 秒 · ${currentRatio()} · ${currentResolution()}`);
